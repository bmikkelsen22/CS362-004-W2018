...
int feastCardEffect(struct gameState* state, int* currentPlayer, int* choice1, int* temphand, int* x) {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      return adventurerCardEffect(&drawntreasure, state, &currentPlayer, temphand, &z, &cardDrawn);
			
    case council_room:
      return councilRoomCardEffect(state, &currentPlayer, &handPos);
			
    case feast:
      return feastCardEffect(state, &currentPlayer, &choice1, temphand, &x);

    case gardens:
      return -1;
    
    case mine:
      return mineCardEffect(state, &currentPlayer, &choice1, &choice2, &handPos, &j);
